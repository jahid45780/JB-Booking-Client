...
const CheckoutForm = ({ bookingInfo, closeModal }) => {
  ...
  const handleSubmit = async event => {
    ...
    // ai code diya user ar taka kata hoy
    ...
    if (paymentIntent.status === 'succeeded') {
      ...
      const paymentInfo = {
       ...
       try{
        //  save payment info in the server
         await saveBookingInfo(paymentInfo)

          // Update room status in db
          await updateStatus(bookingInfo.roomId, true)
          const text = `Your Room Booking SuccessFull Dear 😍 Your ID: ${paymentIntent.id} `
          Swal.fire({
            position: "top-center",
            icon: "success",
            title: (text) ,
